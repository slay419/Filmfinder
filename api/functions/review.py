...
def editReview(review_id, comment, score):
    ...
    conn = sqlite3.connect("users.db")
    cur = conn.cursor() 
    cur.execute(f"UPDATE review SET comment = \"{comment}\", score = {score} where review_id = {review_id};")
    conn.commit()
    conn.close()
    return {"success": "True"}
...
def incrementLikes(review_id):
    ...
    cur.execute(f"UPDATE review SET num_likes = {num_likes} where review_id = {review_id};")
    conn.commit()
    conn.close()
    return {"success": "True"}
